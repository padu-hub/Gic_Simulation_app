...
%   - Substation max 60-min mean |GIC| per event
%   - Transformer winding-1 max 60-min mean |GIC| per event (stored only)
%
% Also generates summary plots (NOT bubble maps):
%   1) Heatmap: substations x events (max 60-min mean)
...
%   results.aggregate.sub.median60  (or max60/mean60)
...
% =========================================================================
    ...
    % subMat:   [nSubs  x nEvents] = max 60-min mean per event per sub
    % transMat: [nTrans x nEvents] = max 60-min mean per event per trans (W1)
    ...
    for i = 1:nEvents
        ...
        % -----------------------------
        % Compute 60-min window in samples using timeVec
        % -----------------------------
        wSamp = windowSamplesFromDatetime(timeVec, minutes(60));

        % -----------------------------
        % Compute max 60-min mean |GIC|
        % -----------------------------
        sub_max60 = maxMovMeanAbs_2D(gicSubs, wSamp); % [nSubs x 1]
        subMat(:,i) = sub_max60(:);

        if ~isempty(gicTransW1)
            trans_max60 = maxMovMeanAbs_2D(gicTransW1, wSamp); % [nTrans x 1]
            transMat(:,i) = trans_max60(:);
        end
        ...
        results.events(i).winSamples60 = wSamp;

        results.events(i).sub_max60  = sub_max60;      % vector, bubble-size input later
        results.events(i).trans_max60_w1 = [];         % keep field consistent
        if ~isempty(gicTransW1)
            results.events(i).trans_max60_w1 = transMat(:,i);
        end
    end
    ...
    % ---------------------------------------------------------------------
    results.matrix.sub_max60_byEvent   = subMat;    % [nSubs x nEvents]
    results.matrix.trans_max60_byEvent = transMat;  % [nTrans x nEvents]

    % ---------------------------------------------------------------------
    % Aggregate "persistent hotspot" metrics (substations)
    % ---------------------------------------------------------------------
    results.aggregate.sub.median60 = nanmedian(subMat, 2);
    results.aggregate.sub.mean60   = nanmean(subMat,   2);
    results.aggregate.sub.max60    = nanmax(subMat,    [], 2);
    results.aggregate.sub.nEvents  = sum(~isnan(subMat), 2);

    % Transformer aggregates stored too (even if you won't bubble-map them)
    if nTrans > 0
        results.aggregate.trans.median60 = nanmedian(transMat, 2);
        results.aggregate.trans.mean60   = nanmean(transMat,   2);
        results.aggregate.trans.max60    = nanmax(transMat,    [], 2);
...
function makeStormSummaryPlots(results)
    subMat = results.matrix.sub_max60_byEvent; % [nSubs x nEvents]
    ...
    % -----------------------------
    figure('Name','GIC Hotspots Heatmap (Max 60-min Mean |GIC|)');
    imagesc(subMatV);
    colorbar;
    xlabel('Event');
    ylabel('Substation Index');
    title('Heatmap: Max 60-min Mean |GIC| per Substation per Event');
    ...
    % -----------------------------
    med60 = results.aggregate.sub.median60;
    [~, idxSort] = sort(med60, 'descend', 'MissingPlacement','last');

    N = min(20, numel(idxSort)); % Top 20 default
    topIdx = idxSort(1:N);

    % -----------------------------
    % Plot 2: Bar chart of Top-N by median60
    % -----------------------------
    figure('Name','Top Hotspot Substations (Median across Events)');
    bar(med60(topIdx));
    grid on;
    ylabel('Median of Event Max 60-min Mean |GIC|');
    ...
    grid on;
    ylabel('Event Max 60-min Mean |GIC|');
