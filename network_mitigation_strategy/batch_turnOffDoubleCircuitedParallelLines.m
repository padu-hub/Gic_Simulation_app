function batch_turnOffDoubleCircuitedParallelLines(app, GICbase)
...
% ----------------------------------------------------------
    ...
    for i = candidateIdx(:).'  % each line = one scenario
        ...
        for sid = 1:nSubs
            ...
            rows(end+1) = makeRow(simID, 'Double Line OFF', app.L(i).Name, i, ... 
    ...
    app.StatusTextArea.Value = [app.StatusTextArea.Value; sprintf('Total time turning  of Double circuited lines: %.2f seconds', elapsedTime)];
    drawnow;
end
