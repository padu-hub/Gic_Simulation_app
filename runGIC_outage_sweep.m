...
function results = runGIC_outage_sweep(app)
...
for j = 1:size(transPool,1)
    ...
    gicMatrix(nLines + j,:) = max(subsNow - baseSubs, [], 1, 'omitnan');
...
[~, idxSort] = sort(runImpact, 'descend');
...
results.gicMatrix = gicMatrix;
results.runNames = runNames;
results.keptSubs = keepSubs;
