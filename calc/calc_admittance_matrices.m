function [Yn,Ye] = calc_admittance_matrices(edges,indices,nodeRes,neutralNodes,S,nBus)


Yn = zeros(max(edges(:))); %admittance matrix
%Yn = zeros(length(unique(edges(:)))); %admittance matrix
...
for k = 1:size(edges,1)
    ...
    nodalY = sum(-1./nodeRes(indices{k}),'omitnan'); % = 1/R_t
...
R = [S(~isnan([S(:).Resistance])).Resistance];

earthingInd = unique(neutralNodes)-nBus;
R = [S(earthingInd).Resistance];
diagYe(unique(neutralNodes))=(1./R)/3; %divide *admittance* by 3 for 3 phase power
