function [V,gic3d,avg_strike,avg_r,std_strike,emag,simple_LV] = calc_line_voltage_integral(ex,ey,L,S,interp_method,compute_coupling)
...
if compute_coupling == 1 || compute_coupling == 3
    avg_strike = zeros(size(V));
    avg_r = zeros(size(V));
    std_strike = zeros(size(V));
    emag = zeros(size(V));
    simple_LV = zeros(size(V));
else
    avg_strike = NaN;
    avg_r = NaN;
    std_strike = NaN;
...
Fex3d = scatteredInterpolant(d.loc(:,1),d.loc(:,2),ex3d(tind(1),:).',interp_method);
Fey3d = scatteredInterpolant(d.loc(:,1),d.loc(:,2),ey3d(tind(1),:).',interp_method);    

Fex1d = scatteredInterpolant(LAT(:),LON(:),ex1d(tind(1),:).',interp_method);
Fey1d = scatteredInterpolant(LAT(:),LON(:),ey1d(tind(1),:).',interp_method);
    ...
    for su = 1:length(su_ind)-1
        ...
        for sidx = 1:nlines
            ...
            if length(s)>1
                ...
                V(indslice,sidx) = nansum(repmat(dx,size(dex1,2),1).*dex1.'+repmat(dy,size(dey1,2),1).*dey1.',2);
            end
