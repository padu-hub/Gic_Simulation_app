...
Mit.Field = {'Resistance','W1'}; % S Fields: Resistance, Latitude, Longitude
...
for i = 1:numel(Mit.Values{1})
    ...
    % Apply All Changes
    % =================================================
    for m = 1:length(Mit.Type)

        val = numel(Mit.Values{m});
    ...
    [Ssim,Lsim,Tsim,latq,lonq] = buildSandboxSimulationNetwork(app);
    ...
    Results(m).Subs = GIC_sandBox.Subs; 
    Results(m).Lines = GIC_sandBox.Lines; 
    Results(m).iInduced = GIC_sandBox.iInduced; 
