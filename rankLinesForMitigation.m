...
for k = 1:numel(weights)
    ...
    allSteps{k} = r.mitigationSteps;
    allGIC{k}   = r.sumGICSubs;
end
try
    tstamp = datetime('now');
    savefname = fullfile(pwd, sprintf('allSteps_allGIC_%s.mat', datestr(tstamp,'yyyymmdd_HHMMSS')));
    save(savefname, 'allSteps', 'allGIC', 'tstamp');
catch
    warning('Failed to save allSteps and allGIC to file.');
end

figure;
hold on

% hard-coded distinct colors for 11 lines (RGB)
colors = [
    ...
    0.8000 0.6000 0.7000; % pinkish
    0.2000 0.2000 0.2000; % dark gray
    0.1000 0.6000 0.1000; % dark green
    0.6000 0.3000 0.0000; % brown
];

nLines = numel(weights);
if nLines > size(colors,1)
    error('Not enough predefined colors for %d lines.', nLines);
end

for k = 1:nLines
    plot(1:numel(allGIC{k}), allGIC{k}, ...
        'LineWidth', 2, ...
        'Color', colors(k,:), ...
        'DisplayName', sprintf('%d%%', weights(k)));
end
...
function out = normalize0tox(v,x)
    out = zeros(size(v));
    if all(isnan(v) | v==0)
        out(:) = 0;
        return;
