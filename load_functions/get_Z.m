...
elseif option_flag == 2  % Uniform halfspace impedance
    data = load(zFile);
    d = data.d;
    ...
    d.Z(:,[1 4],:) = 0;
    for i = 1:d.ns
        Zval = ((1+1i)/sqrt(2)) * sqrt((2*pi*d.f'*mu0) / sigma);
        d.Z(:,2,i) = Zval;
        d.Z(:,3,i) = -Zval;
    end
...
elseif option_flag == 3  % Trichtchenko 1-D zones
    zn = load_trich_zones;
...
elseif option_flag == 4  % Data space method (legacy path hardcoded, consider updating)
    data = load('AB_BC_MT_DATA_526_sites_230321.mat');
    d = data.d;
    disp(['Loaded MT Impedance File: ', zFile]);

elseif option_flag == 5  % 1-D forward model
    data = load(zFile);
    d = data.d;
...
elseif option_flag == 6  % MARE2DEM 2-D model (still has fixed cd — best to refactor)
    data = load(zFile);
    d = data.d;

    % You can replace this hardcoded cd with a config value or skip entirely
