function [nodePairs,nodeRes,nodeInd,edges,indices,neutralNodes,autoind,nBus] = get_nodePairs(L,T,S)
%Get number of buses in the network by finding max bus number from list of
%buses
%nBus = max([[T.W1Bus] [T.W2Bus] [L.fromBus] [L.toBus]]);
uniqueBus = unique([[T.W1Bus] [T.W2Bus] [L.fromBus] [L.toBus]]);
nBus = length(uniqueBus);

for i = 1:nBus
    busIndex = find([T.W1Bus]==uniqueBus(i));
    for j = 1:length(busIndex)
        T(busIndex(j)).W1Bus = i;
    end

    busIndex = find([T.W2Bus]==uniqueBus(i));
    for j = 1:length(busIndex)
        T(busIndex(j)).W2Bus = i;
    end

    busIndex = find([L.fromBus]==uniqueBus(i));
    for j = 1:length(busIndex)
        L(busIndex(j)).fromBus = i;
    end

    busIndex = find([L.toBus]==uniqueBus(i));
    for j = 1:length(busIndex)
        L(busIndex(j)).toBus = i;
    end
end
