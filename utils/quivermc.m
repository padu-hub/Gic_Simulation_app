function [hvectors,hpatch,cb] = quivermc(lat,lon,u,v,varargin)
...
%assert(isscalar(lat) || isvector(lat),'Input lat, lon, u, and v must be a grid.') 
%assert(isvector(lat),'Input lat, lon, u, and v must be a grid.') 
...
hpatch = patch([xend; x2; x3],[yend; y2; y3],veccol,'FaceColor',veccol,'EdgeColor',veccol);
