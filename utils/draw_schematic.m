function draw_schematic(b, mode, subName, iSub, ax, L, T, GIC, specTimeIndex, timeVec)
%DRAW_SCHEMATIC  One-page substation view: lines in/out, xfmrs, and ground flow.
% Sign rule used here: for wye/auto windings, +GIC => to ground.
%
% Inputs:
...
%   h        : handles (lines/text), optional for custom tweaks
    ...
    for i = 1:numel(L)
        ...
        if isfield(L(i), 'Voltage') % Assuming 'Voltage' field exists
            if L(i).Voltage == 240
                lineColor = 'r'; % Red for LV (240kV)
            elseif L(i).Voltage == 500
                lineColor = 'b'; % Blue for HV (500kV)
            else
                lineColor = cLine; % Default color
            end
        else
            lineColor = cLine; % Default color if no voltage info
        end

        if isTo % at the "to" end, + means INCOMING to this node
            y = yL; yL = yL - dy;
            plot(ax,[1 4.2],[y y],'Color',lineColor,'LineWidth',2);
            draw_arrow(ax, 3.6, y, 4.2, y, lineColor);    % toward node
            text(ax, 1.1, y+0.25, sprintf('%s\n%.1f A', safeName(L(i)), I), ...
                 'Color',lineColor,'FontSize',9,'HorizontalAlignment','left');
            inSum = inSum + I;  % toward node
        else % isFrom; + means OUTGOING from this node
            y = yR; yR = yR - dy;
            plot(ax,[5.8 9],[y y],'Color',lineColor,'LineWidth',2);
            draw_arrow(ax, 5.8, y, 6.4, y, lineColor);    % away from node
            text(ax, 8.9, y+0.25, sprintf('%.1f A\n%s', I, safeName(L(i))), ...
                 'Color',lineColor,'FontSize',9,'HorizontalAlignment','right');
